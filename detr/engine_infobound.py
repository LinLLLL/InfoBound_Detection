...
from datasets.eval import CocoEvaluatorNEW
...
from visualize import visualize_prediction_results, visualize_prediction_results_ood
...
def train_one_epoch(model: torch.nn.Module, swav_model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    postprocessors=None, samples_uda=None, uda_labels=None, ref_boxes_uda=None, threshold=None, args=None):
    assert postprocessors is not None
    # assert (samples_uda is not None and uda_labels is not None)
    model.train()
    criterion.train()
    metric_logger = utils.MetricLogger(delimiter="  ")
    metric_logger.add_meter('lr', utils.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    ...
    # uda_labels = torch.cat([uda_labels[0], uda_labels[1]], 0)

    if uda_labels is not None and samples_uda is not None and len(samples_uda) == 1:
        uda = []
        random_index = random.sample(range(len(samples_uda[0])), 2)  # todo: args.batch_size # 4
        for i, (uda_samples, _) in enumerate(samples_uda[0]):
            if i in random_index:
                uda.append(uda_samples)
        # uda_labels = uda_labels[-1][random_index]
        samples_uda_ = uda

    ## train-one-epoch ##
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ################################### sampling uda #######################################
        if samples_uda is not None and uda_labels is not None and len(samples_uda) == 2:
            uda = []
            random_index = random.sample(range(len(samples_uda[0])), 2)  # todo: args.batch_size // 2
            for i, (uda_samples, _) in enumerate(samples_uda[0]):
                if i in random_index:
                    uda.append(uda_samples)
            # uda_labels_sample0 = uda_labels[0] #[random_index]

            random_index = random.sample(range(len(samples_uda[1])), 2)  # todo: args.batch_size // 2
            for i, (uda_samples, _) in enumerate(samples_uda[1]):
                if i in random_index:
                    uda.append(uda_samples)
            # uda_labels_sample1 = uda_labels[1] # [random_index]
            # uda_labels = torch.cat([uda_labels_sample0, uda_labels_sample1], 0)
            samples_uda_ = uda
        ################################### sampling uda #######################################

        if samples_uda_ is not None and uda_labels is not None:
            outputs_uda, pen_features, pred_boxes_uda = evaluate_uda(model, criterion, postprocessors, samples_uda_, device)
        energy_uda = -torch.logsumexp(outputs_uda, dim=1)
        uda_labels = (1 * (energy_uda <= threshold))

        samples, targets = to_cuda(samples, targets, device)
        outputs = model(samples, epoch=epoch)
        if swav_model is not None:
            with torch.no_grad():
                for elem in targets:
        ...
        # uda_labels is generated by the model at epoch 0
        loss_dict = criterion(outputs, targets, outputs_uda, pred_boxes_uda, uda_labels, ref_boxes_uda, pen_features)
        # loss_dict = criterion(outputs, targets)
        ...
        metric_logger.update(lr=optimizer.param_groups[0]["lr"])
        metric_logger.update(grad_norm=grad_total_norm)
        # threshold = metric_logger.meters['energy_id:_unscaled'].median
...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, dataset, args):
    ...
    coco_evaluator = (CocoEvaluator if hasattr(base_ds, "getImgIds") else VocEvaluator)(base_ds, iou_types)
...
@torch.no_grad()
def evaluate_ood_id(args, model, criterion, postprocessors, data_loader, base_ds, device, output_dir, address, dataset, vis_prediction_results, epoch):
    model.eval()
    # original_siren_flag = criterion.args.siren_evaluate
    # if original_siren_flag:
    #     criterion.args.siren_evaluate = False
    ...
    if not args.maha_train:
        np.save(address + 'id-logits-{}.npy'.format(epoch), all_logits)
        np.save(address + 'id-pen-{}.npy'.format(epoch), pen_features)
        if project_features is not None:
            np.save(address + 'id-pro-{}.npy'.format(epoch), project_features)
        if sampling_cls is not None:
            np.save(address + 'id-sampling-{}.npy'.format(epoch), sampling_cls)
        if out_godin_h is not None:
            np.save(address + 'id-godin-{}.npy'.format(epoch), out_godin_h)
    else:
        np.save(address + 'id-logits_maha_train-{}.npy'.format(epoch), all_logits)
        np.save(address + 'id-pen_maha_train-{}.npy'.format(epoch), pen_features)
        if project_features is not None:
            np.save(address + 'id-pro_maha_train-{}.npy'.format(epoch), project_features)
        if sampling_cls is not None:
            np.save(address + 'id-sampling_maha_train-{}.npy'.format(epoch), sampling_cls)
        if out_godin_h is not None:
            np.save(address + 'id-godin_maha_train-{}.npy'.format(epoch), out_godin_h)
    # gather the stats from all processes
    metric_logger.synchronize_between_processes()
    print("Averaged stats:", metric_logger)
    if coco_evaluator is not None:
...
def evaluate_ood_ood(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, address, dataset, vis_prediction_results, epoch=-1):
    model.eval()
    criterion.eval()
    all_logits = None
    pen_features = None
    ...
    for samples, targets in data_loader: #metric_logger.log_every(data_loader, 10, header):
        ...
        if all_logits is None:
            all_logits = results[0]['logits_for_ood_eval'].view(-1,
                                                                results[0]['logits_for_ood_eval'].shape[-1]).cpu().data.numpy()
        else:
            temp = results[0]['logits_for_ood_eval'].view(-1,
                                                          results[0]['logits_for_ood_eval'].shape[-1]).cpu().data.numpy()
            all_logits = np.concatenate((all_logits, temp), 0)
        ...
        if vis_prediction_results:
            visualize_prediction_results_ood(samples, results, output_dir, targets, True)
        continue
        loss_dict = criterion(outputs, targets)
        weight_dict = criterion.weight_dict
    ...
    if dataset == 'openimages_ood_val':
        np.save(address + 'ood-open-logits-{}.npy'.format(epoch), all_logits)
        np.save(address + 'ood-open-pen-{}.npy'.format(epoch), pen_features)
    else:
        np.save(address + 'ood-logits-{}.npy'.format(epoch), all_logits)
        np.save(address + 'ood-pen-{}.npy'.format(epoch), pen_features)
    if project_features is not None:
        if dataset == 'openimages_ood_val':
            np.save(address + 'ood-open-pro-{}.npy'.format(epoch), project_features)
        else:
            np.save(address + 'ood-pro-{}.npy'.format(epoch), project_features)
    if sampling_cls is not None:
        if dataset == 'openimages_ood_val':
            np.save(address + 'ood-open-sampling-{}.npy'.format(epoch), sampling_cls)
        else:
            np.save(address + 'ood-sampling-{}.npy'.format(epoch), sampling_cls)
    if out_godin_h is not None:
        if dataset == 'openimages_ood_val':
            np.save(address + 'ood-open-godin-{}.npy'.format(epoch), out_godin_h)
        else:
            np.save(address + 'ood-godin-{}.npy'.format(epoch), out_godin_h)
    return None, None
    # gather the stats from all processes
    metric_logger.synchronize_between_processes()
    print("Averaged stats:", metric_logger)
...
def evaluate_ood_id_speckle(args, model, criterion, postprocessors, data_loader, base_ds, device, output_dir, address, dataset, vis_prediction_results):
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        # print(results[0]['logits_for_ood_eval'].shape)
        if all_logits is None:
            all_logits = results[0]['logits_for_ood_eval'].view(-1, results[0]['logits_for_ood_eval'].shape[2]).cpu().data.numpy()
        else:
            temp = results[0]['logits_for_ood_eval'].view(-1, results[0]['logits_for_ood_eval'].shape[2]).cpu().data.numpy()

            all_logits = np.concatenate((all_logits, temp), 0)


        if pen_features is None:
            pen_features = results[0]['pen_features'].view(-1, results[0]['pen_features'].shape[2]).cpu().data.numpy()
        else:
            temp = results[0]['pen_features'].view(-1, results[0]['pen_features'].shape[2]).cpu().data.numpy()
...
def evaluate_uda(model, criterion, postprocessors, uda_samples, device):
    model.eval()
    criterion.eval()
    all_logits = None
    pen_features = None
    ...
    for samples in uda_samples:
        samples = samples.to(device)
        outputs = model(samples)

        orig_target_sizes = torch.stack([torch.tensor([224, 224]) for i in range(outputs['pred_boxes'].shape[0])], dim=0)  # this is just a placeholder and do not influence logits

        results = postprocessors['bbox'](outputs, orig_target_sizes.cuda())

        if all_logits is None:
            all_logits = results[0]['logits_for_ood_eval'].view(-1, results[0]['logits_for_ood_eval'].shape[-1])
        else:
            temp = results[0]['logits_for_ood_eval'].view(-1, results[0]['logits_for_ood_eval'].shape[-1])
            all_logits = torch.cat((all_logits, temp), 0)

        if pen_features is None:
            pen_features = results[0]['pen_features'].view(-1, results[0]['pen_features'].shape[2])
        else:
            temp = results[0]['pen_features'].view(-1, results[0]['pen_features'].shape[2])
            pen_features = torch.cat((pen_features, temp), 0)

        if pred_boxes is None:
            pred_boxes = results[0]['boxes_for_ood_eval'].view(-1, results[0]['boxes_for_ood_eval'].shape[-1])
        else:
            temp = results[0]['boxes_for_ood_eval'].view(-1, results[0]['boxes_for_ood_eval'].shape[-1])
            pred_boxes = torch.cat((pred_boxes, temp), 0)

    return all_logits, pen_features, pred_boxes
