...
class SetInfoBoundCriterion(nn.Module):
    """ This class computes the loss for DETR.
    The process happens in two steps:
        1) we compute hungarian assignment between ground truth boxes and the outputs of the model
        2) we supervise each pair of matched ground-truth / prediction (supervise class and box)
    ...
    def loss_mm(self, outputs, targets, indices, num_boxes, uda=None):
        """maximum-mutual-information and minimum-conditional-information loss for ood-aware object detection and ood-generalization
        This is a self-supervised-style regularization loss, the id object bbox and ood object bbox are selected
        by the energy score, the lager the energy score, the more the object OOD-ness.
        The cosine similarity between id proposal and ood proposals is minimized to reduce their mutual-information,
        the energy score of the ood proposals is maximized to increase the conditional-entropy, which focuses on detect ood objects.
        """

        assert 'pred_logits' in outputs
        assert 'bbox_embd' in outputs
        src_logits = outputs['pred_logits']

        bbox_embd = outputs['pen_features']  # outputs['bbox_embd']
        energy = -torch.logsumexp(src_logits, dim=2)

        src_boxes = outputs['pred_boxes'].flatten(0, 1)  # [batch_size * num_queries, 4]
        target_boxes = torch.cat([v["boxes"] for v in targets])
        # print(src_boxes.shape, target_boxes.shape)
        giou = box_ops.generalized_box_iou(box_ops.box_cxcywh_to_xyxy(src_boxes),
                                           box_ops.box_cxcywh_to_xyxy(target_boxes))

        if giou.reshape(src_logits.shape[0], src_logits.shape[1], -1).shape[-1] != 0:
            # max_giou = torch.max(giou.reshape(src_logits.shape[0], src_logits.shape[1], -1), -1)[0]
            max_giou = torch.max(giou, -1)[0]
        else:
            return  {'loss_mm': torch.tensor(0.).cuda()}

        id = max_giou > 0.8  # 0.5 for bdd
        ood = max_giou < -1000
        ...
        if id.any():
            ...
            energy_id = torch.mean(torch.sigmoid(energy[id]))
            energy_ood = torch.mean(torch.sigmoid(-energy[max_giou < 0.2]))
            loss_energy = energy_id + energy_ood
        else:
            loss_energy = torch.tensor(0.).cuda()

        if uda is not None:
            outputs_uda, pred_boxes_uda, uda_labels, ref_boxes_uda, bbox_embd_uda = uda
            uda_logits = outputs_uda

            giou_uda = box_ops.generalized_box_iou(box_ops.box_cxcywh_to_xyxy(pred_boxes_uda),
                                              box_ops.box_cxcywh_to_xyxy(ref_boxes_uda))  # not used

            id_uda = uda_labels == 1 
            ood_uda = uda_labels == 0 

            energy_uda = -torch.logsumexp(uda_logits, dim=1) 

            id_all = torch.cat([id, id_uda], 0)
            ood_all = torch.cat([ood, ood_uda], 0)
            id_length = torch.nonzero(id > 0).shape[0]
            
            bbox_embd = torch.cat([bbox_embd.reshape(-1, bbox_embd.shape[-1]), bbox_embd_uda], 0)
            bbox_embd_new = bbox_embd[(torch.cat([1 * id, 1 * id_uda + 1 * ood_uda], -1)> 0)]
            id_new = torch.ones([1, id_length]).cuda()
            id_new = torch.cat([id_new, id_uda.reshape(1, -1) * 1], 1)

            ood_new = torch.cat([1 - torch.ones([1, id_length]).cuda(), ood_uda.reshape(1, -1) * 1], 1)
            # ############################## cosine similarity for minimizing mutual information ###################################
            bbox_embd_new =  bbox_embd_new.softmax(dim=-1)

            S = bbox_embd_new @ bbox_embd_new.transpose(-1, -2)  # self-similarity [all, all]
            mask_id_ood = (1. * ood_new).unsqueeze(2) @ (1. * id_new).unsqueeze(1)
            
            mutual_info1 = (S * mask_id_ood).sum(dim=2)
            mask_id_id = (1. * id_new).unsqueeze(2) @ (1. * id_new).unsqueeze(1)
            mutual_info2 = (S * mask_id_id).sum(dim=2)
            mutual_info1 = mutual_info1 / (mutual_info1.norm(dim=-1, keepdim=True))
            mutual_info2 = mutual_info2 / (mutual_info2.norm(dim=-1, keepdim=True))
            mutual_info =  torch.exp(mutual_info1).mean() / (1e-6 + torch.exp(mutual_info2)).mean()

            # ############################## maximizing condotional information #############################
            loss_uda = torch.tensor(0.).cuda()
            if id_uda.any():
                loss_uda_id = torch.mean(torch.sigmoid(energy_uda[id_uda]))
                loss_uda = loss_uda_id
            if ood_uda.any():
                loss_uda_ood = torch.mean(torch.exp(-100*mutual_info1[:, id_length:][ood_uda.reshape(1, -1)]) * torch.sigmoid(-energy_uda[ood_uda])) #
                loss_uda = loss_uda_ood + loss_uda
            # ##########################################################################################
            if id.any() and id_all.any() and ood_all.any():
                losses = {'loss_mm':  mutual_info + loss_uda + loss_energy} # mutual_info + loss_uda + loss_energy
            else:
                losses = {'loss_mm': loss_energy}
            losses['energy_id:'] = torch.quantile(energy_uda[id_uda], 0.95) if id_uda.any() else torch.quantile(energy[id], 0.95)
            losses['energy_ood:'] = torch.quantile(energy_uda[ood_uda], 0.5) if ood_uda.any() else torch.tensor(0.).cuda()
            # losses['loss_energy:'] = loss_energy
            losses['loss_MMI:'] = mutual_info
        else:
            losses = {'loss_mm': loss_energy}
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes, **kwargs):
        loss_map = {
            ...
            'boxes': self.loss_boxes,
            'masks': self.loss_masks,
            'mm_energy': self.loss_mm
        }
        assert loss in loss_map, f'do you really want to compute {loss} loss?'
        return loss_map[loss](outputs, targets, indices, num_boxes, **kwargs)

    def forward(self, outputs, targets, outputs_uda=None, pred_boxes_uda=None, uda_labels=None, ref_boxes_uda=None, pen_features=None):
        """ This performs the loss computation.
        Parameters:
             outputs: dict of tensors, see the output specification of the model for the format
             targets: list of dicts, such that len(targets) == batch_size.
        ...
        for loss in self.losses:
            kwargs = {}
            if loss == 'mm_energy' and outputs_uda is not None and self.args.mm_energy:
                kwargs['uda'] = (outputs_uda, pred_boxes_uda, uda_labels, ref_boxes_uda, pen_features)
            losses.update(self.get_loss(loss, outputs, targets, indices, num_boxes, **kwargs))
