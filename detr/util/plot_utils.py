...
def plot_prediction(image, boxes, logits, ax=None, plot_prob=True):
    bboxes_scaled0 = rescale_bboxes(boxes[0], list(image.shape[2:])[::-1]).cpu().numpy()
    ...
    if ax is None:
        ax = plt.gca()
    plot_results(image[0].permute(1, 2, 0).cpu().numpy(), probas[keep], bboxes_scaled0[keep], ax, plot_prob=plot_prob)
